...
class Game {
  ...
  constructor({
    playUser,
    playComp,
    statistics: { quantityShots, listShips, shipOfList, classNameDeadShip },
    components: {
      containerGame,
      cell,
      btnRnd,
    ...
    if (cellElement) {
      ...
      this.delay =
        animDurationStr
          .replace(/(\d+)(s|ms)/gi, (m, p1, p2) => {
            return +p1 * (p2.toLowerCase() === 's' ? 1000 : 1);
          })
          .split(',')
          .reduce((sum, num) => (sum += +num), 0) * 1.5;
    }
    ...
    btnResetGameEl &&
      btnResetGameEl.addEventListener('click', async () => {
        /**
        ...
        const isResolve =
          elementCloseModal && elementCloseModal.value === 'yes';
        if (isResolve) {
          this.resetGame();
        }
      });
     ...
     * @type {HTMLButtonElement}
     */
    this.soundBackgroundToggle = document.querySelector(
      soundBackgroundToggleSelector
    );
    /**
     ...
     * @type {HTMLButtonElement}
     */
    this.soundEffectsToggle = document.querySelector(
      soundEffectsToggleSelector
    );
    /**
    ...
    this.soundEffectsToggle?.addEventListener('click', (e) =>
      this.toggleSoundEffect(e)
    );

    if (this.soundBackground) {
      this.soundBackground.loop = true;
      this.soundBackgroundToggle?.addEventListener('click', (e) =>
        this.toggleSoundBg(e)
      );
    }
  }
  /**
  ...
  async shotUser() {
    ...
    await this.makeAnimateShot(
      this.playUser,
      this.playComp,
      this.playUser.getCoordCellOfShipRnd(),
      coord
    );
    ...
    this.updateQuantityShots(
      this.quantityShotsHumanEl,
      ++this.quantityShotsHuman
    );
  ...
  async shotComp() {
    ...
    await this.makeAnimateShot(
      this.playComp,
      this.playUser,
      this.playComp.getCoordCellOfAreaRnd(),
      coord
    );
    if (!this.isGaming) return null;
  ...
  async makeAnimateShot(shooter, target, shotFrom, shotTo) {
    ...
    // координаты откуда выстрел
    const { left: leftFrom, top: topFrom } = PlayArea.getAbsoluteCoordinates(
      target.containerCell,
      shooter.area[ii][kk].cellHtml
    );
    // координаты куда выстрел
    const { left: leftTo, top: topTo } = PlayArea.getAbsoluteCoordinates(
      target.containerCell,
      target.area[i][k].cellHtml
    );
    ...
    const distance = Math.sqrt(
      (parseFloat(topFrom) - parseFloat(topTo)) ** 2 +
        (parseFloat(leftFrom) - parseFloat(leftTo)) ** 2
    );
    const draw = (progress) => {
      const currentProgressTop = progress * distanceTop;
      const currentProgressLeft = progress * distanceLeft;
      const scale =
        1 + 4 * (progress * distance < distance / 2 ? progress : 1 - progress);
      const rotate = (progress * 720) % 360;

      cannonballEl.style.top = parseFloat(topFrom) + currentProgressTop + '%';
      cannonballEl.style.left =
        parseFloat(leftFrom) + currentProgressLeft + '%';
  ...
  updateDeadShips(element, sizeShip) {
     ...
     * @type {HTMLElement}
     */
    const shipEl = ([...element?.children[sizeShip - 1]?.children] || []).find(
      (elem) => !elem.classList.contains(this.classNameDeadShip)
    );
    shipEl.innerHTML = `
  ...
  async start(mode = 'random') {
    ...
    switch (mode) {
      ...
      default:
        this.currentShot =
          randomInteger(1, 100) % 2 ? this.shotUser : this.shotComp;
        break;
    }
    this.isGaming = true;
    /**
