...
class Area {
  constructor() {
    ...
    this.createPlayArea();
    this.totalShips = Object.values(this.schemeShips).reduce(
      (total, num) => total + num,
      0
    );
  }
  ...
  getRandomEmptyPoint() {
    ...
    do {
      ...
      if (setPoints.add(`${i}:${k}`).size >= maxIteration)
        throw new GridError(
          'Error searching for a free cell. All cells are occupied!'
        );
  ...
  getRandomDirect(excludeArr = []) {
    ...
    const isAllOptionsExcluded = directArr.every((val) =>
      excludeArr.includes(val)
    );
    ...
    const setBorder = (i, k) => {
      ...
      for (let ii = iMin; ii <= iMax; ii++) {
        for (let kk = kMin; kk <= kMax; kk++) {
          if (
            /* this.area[ii][kk].cell !== this.emptyCell || */ track.find(
              ({ i, k }) => ii === i && kk === k
            )
          )
            continue;
