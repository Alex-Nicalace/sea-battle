  ...
  static showModal(modalEl, { content } = {}) {
    return new Promise((res, rej) => {
      ...
      if (!modalEl.showModal)
        rej(new Error('The element HTMLDialog is not specified'));
      ...
      const closeModal = (element = undefined) => {
        ...
        trigersHideModal.forEach((trigger) =>
      ...
      trigersHideModal.forEach((trigger) =>
        trigger.addEventListener('click', () => closeModal(trigger))
      );
      ...
      if (modalBody) {
        const control = modalBody.querySelectorAll(
